...
$new_app.Icon = "https://github.com/OptechX/library.apps.images/raw/main/$($Env:applicationCategory)/Office%20365/Microsoft%20Teams/icon.png"
...
{
    $version_page = Invoke-WebRequest -Uri "https://teams.microsoft.com/downloads/DesktopUrl?env=production&plat=windows"
    $version_url = $version_page.Content.Trim()
    $new_app.Version = $version_url | 
        ...
        Select-Object -ExpandProperty Matches -First 1 |
        Select-Object -ExpandProperty Value
    $new_app.Filename = "Teams_windows_x64.msi"
    $download_url = "http://aka.ms/teams64bitmsi"
    $download_link = (Invoke-WebRequest -Uri $download_url -Method Head -MaximumRedirection 0 -SkipHttpErrorCheck -ErrorAction Ignore | Select-Object -ExpandProperty Headers).Location[0]
