...
# DotNet/HostingBundle
$pRoot = "./winget-pkgs/manifests/m/Microsoft/DotNet/HostingBundle"
                    ...
                    foreach ($arch in $yamlInstaller.Installers.Architecture)
                    {
                        switch ($arch)
                        {
                            "x64" {
                                ...
                                $new_pkg.cpuArch = @("x64")
                                $new_pkg.homepage = if ($yamlLocale.PackageUrl) { $yamlLocale.PackageUrl } elseif ($yamlLocale.PublisherUrl) { $yamlLocale.PublisherUrl } elseif ($yamlLocale.ReleaseNotesUrl) { $yamlLocale.ReleaseNotesUrl } else { "" }
                                $new_pkg.icon = $iconUri
                                $new_pkg.docs = if ($yamlLocale.PackageUrl) { $yamlLocale.PackageUrl } elseif ($yamlLocale.PublisherUrl) { $yamlLocale.PublisherUrl } elseif ($yamlLocale.ReleaseNotesUrl) { $yamlLocale.ReleaseNotesUrl } else { "" }
