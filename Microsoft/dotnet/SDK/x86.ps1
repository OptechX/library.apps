...
$new_app.Name = ".NET SDK"
$new_app.Lcid = @("en-US")
$new_app.CpuArch = @("x86")
$new_app.Homepage = "https://dot.net/core"
$new_app.Copyright = "Copyright (c) .NET Foundation and Contributors"
$new_app.Icon = "https://github.com/OptechX/library.apps.images/raw/main/$($env:applicationCategory)/dotnet/SDK/icon.png"
...
$new_app.Tags = @("microsoft",".net","dotnet","dot-net","runtime","aspnet","asp.net","core","sdk")
...
{
    $version_url = 'https://dotnet.microsoft.com/en-us/download/dotnet/7.0'
    $x = Get-MSDotNetPackageDetails -VersionUrl $version_url `
        -HtmlPattern '<button data-toggle="collapse".*<\/button>' `
        -VersionPattern '<h3.*>SDK\s7\.([\d{1,}\.]+)<\/h3>' `
        -SubsFilename "dotnet-sdk-<REPLACE>-win-x86.exe" `
        -AbsoluteUrl "https://dotnet.microsoft.com/en-us/download/dotnet/thank-you/sdk-<REPLACE>-windows-x86-installer"
    $new_app.Version = $x[0]
    $new_app.Filename = $x[1]
    $new_app.AbsoluteUri = $x[2]
    $new_app.Executable = 'exe'
    $new_app.InstallArgs = '/passive /norestart'
    #$new_app.DisplayName = "Microsoft Windows AspNet Core Runtime - $($new_app.Version) (x86)"
}
