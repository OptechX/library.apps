...
$new_app.Name = ".NET 6 Desktop Runtime"
$new_app.Lcid = @("EN_US")
$new_app.CpuArch = @("")
$new_app.Homepage = "https://dot.net/core"
$new_app.Copyright = "Copyright (c) .NET Foundation and Contributors"
$new_app.Icon = "https://github.com/OptechX/library.apps.images/raw/main/$($Env:applicationCategory)/dotnet-6.0/xxxxx/icon.png"
...
else
{
    ...
    $new_app.Version = $download_page.Content |
        Select-String -Pattern '<button data-toggle="collapse".*<\/button>' |
        Select-Object -ExpandProperty Matches -First 1 | 
        Select-String -Pattern '6\.([\d{1,}\.]+)' |
        Select-Object -ExpandProperty Matches -First 1 |
        Select-Object -ExpandProperty Value
    $new_app.Filename = "windowsdesktop-runtime-$($new_app.Version)-win-x86.exe"
    $rgx = "<a href=`"https:\/\/..`*windowsdesktop-runtime-$($new_app.Version)-win-x86.exe"
    $new_app.AbsoluteUri = Invoke-WebRequest -Uri "https://dotnet.microsoft.com/en-us/download/dotnet/thank-you/runtime-desktop-$($new_app.Version)-windows-x86-installer" | 
        Select-Object -ExpandProperty Links | 
        Where-Object -FilterScript {$_.outerHtml -match $rgx}
    $new_app.Executable = 'exe'
    $new_app.InstallArgs = '/passive /norestart'
    $new_app.DisplayName = "Microsoft Windows Desktop Runtime - $($new_app.Version) (x86)"
}
