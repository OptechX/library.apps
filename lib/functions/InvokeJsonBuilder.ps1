function Invoke-JsonBuilder {
    [CmdletBinding()]
    param (
        [System.Array]$versionMajor
    )
    
    # build version major
    $versionList = New-Object System.Collections.Generic.List[Version]
    $versionList2 = New-Object System.Collections.Generic.List[string]
    ...
    if ($versionList2.Count -ge 1)
    {
        $sortedVersions = $versionList2 | Sort-Object -Descending
        $highestVersion = $sortedVersions[0]
        $currentVersion = [string]$highestVersion
