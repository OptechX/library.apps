...
$new_app.Icon = "https://github.com/OptechX/library.apps.images/raw/main/$($Env:applicationCategory)/Mozilla/Firefox%20Nightly/icon.svg".Replace(" ","%20")
...
$new_app.Tags = @("browser","mozilla","firefox","nightly","dev","canary","edition")
$new_app.Summary = "Nightly build of Mozilla Firefox."
...
else
{
    ...
    $rgx = "download.mozilla.*product=firefox-nightly.*-msi.*(&amp;|&)os=win64(&amp;|&)lang=en-US"
    ...
    $new_app.Version = ((Split-Path -Path $response.Result.RequestMessage.RequestUri.OriginalString -Leaf) | 
        Select-String -Pattern '-\d{3}\.([0-9a-zA-Z\.]+)\.en-US' | 
        Select-Object -ExpandProperty Matches -First 1 |
        Select-String -Pattern '\d{3}\.([0-9a-zA-Z\.]+)' | 
        Select-Object -ExpandProperty Matches -First 1 | 
        Select-Object -ExpandProperty Value).Replace('.en','')
